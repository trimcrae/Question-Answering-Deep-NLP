...
def run(load_train_dir, save_train_dir):
	...
	attention_c = tf.reduce_max(matrix, axis = 1)
	attention_c = tf.expand_dims(attention_c, axis = 2)

	attention_q = tf.reduce_max(matrix, axis = 2)
	attention_q = tf.expand_dims(attention_q, axis = 2)

	filtered_c = input_c_placeholder*attention_c
	filtered_q = input_q_placeholder*attention_q

	outputs_q, state_q = tf.nn.bidirectional_dynamic_rnn(LSTM_cell_q, LSTM_cell_q, filtered_q, srclen_q, scope = scope_q, time_major = False, dtype = tf.float32)
