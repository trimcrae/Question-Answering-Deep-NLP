...
def run(load_train_dir, save_train_dir):
	...
	LSTM_cell_pass = tf.nn.rnn_cell.BasicLSTMCell(num_units = hidden_size)
	...
	print("filtered_c shape", filtered_c.get_shape())
	...
	intermediate_transformed = tf.transpose(intermediate, [0,2,1])
	print("intermediate shape", intermediate.get_shape())
	print("intermediate transformed shape", intermediate_transformed.get_shape())

	outputs_c, state_c = tf.nn.bidirectional_dynamic_rnn(LSTM_cell_pass, LSTM_cell_pass, intermediate_transformed, srclen_c, scope = scope_pass, time_major = False, dtype = tf.float32)
