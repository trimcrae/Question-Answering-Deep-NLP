...
def run(load_train_dir, save_train_dir):
	...
	intermediate_2 = tf.reduce_mean(intermediate, axis = 1)

	print("shape of intermediate_2")
	print(intermediate_2.get_shape())

	W_s = tf.Variable(tf.truncated_normal([max_c_words, max_c_words]))
	b_s = tf.Variable(tf.constant(.1, shape=[max_c_words]))
	...
	intermediate_2 = tf.matmul(intermediate_2, W_s) + b_s

	print("shape of intermediate_2")
	print(intermediate_2.get_shape())


	p_s_true = intermediate_2   #starting place probabilities
	...
	W_l = tf.Variable(tf.truncated_normal([hidden_size*2, 1]))
	b_l = tf.Variable(tf.constant(1.0, shape=[1]))
	...
	step_1 = tf.matmul(intermediate, W_l)
	step_2 = step_1 + b_l
