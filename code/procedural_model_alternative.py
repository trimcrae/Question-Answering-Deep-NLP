...
def run(load_train_dir, save_train_dir):
	...
	outputs_c, state_c = tf.nn.bidirectional_dynamic_rnn(LSTM_cell_c, LSTM_cell_c, input_c_placeholder, srclen_c, scope = scope_c, time_major = False, dtype = tf.float32)
...
def my_f1_score(s_pred, e_pred, s_true, e_true):
	...
	if ((start_pred[i] > end_true[i]) or (start_true[i] > end_pred[i])):
		...
		num_pred = end_pred[i] - start_pred[i] + 1
		num_true = end_true[i] - start_true[i] + 1
		num_same = overlap	
