...
def run(load_train_dir, save_train_dir):
	...
	dropout = tf.placeholder(tf.float32,  shape = (), name='dropout_placeholder')

	print("shape of dropout")
	print(dropout.get_shape())

	intermediate = tf.nn.dropout(intermediate, dropout)
	...
	for j in range (num_epochs):  #epochs
		...
		for i in range(num_batches):  
			...
			_, batch_error, p_s_true_ = sess.run([train_step, avgLoss, p_s_true], feed_dict={input_q_placeholder: batch_q, input_c_placeholder: batch_c, start_answer_placeholder: start_batch, end_answer_placeholder: end_batch, mask_q_placeholder: batch_mask_q, mask_c_placeholder: batch_mask_c, dropout: .5})			
		...
		tf.add_to_collection('vars2', dropout)
		...
		start_pred, end_pred = sess.run([a_s, a_e], feed_dict={input_q_placeholder: test_q, input_c_placeholder: test_c, mask_q_placeholder: batch_mask_q, mask_c_placeholder: batch_mask_c, dropout: 1})
	...
	start_pred_val, end_pred_val = sess.run([a_s, a_e], feed_dict={input_q_placeholder: test_q, input_c_placeholder: test_c, mask_q_placeholder: batch_mask_q, mask_c_placeholder: batch_mask_c, dropout: 1})
