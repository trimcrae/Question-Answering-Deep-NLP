...
def run(load_train_dir, save_train_dir):
	...
	print("Input q placeholder shape:", input_q_placeholder.get_shape())
	...
	val, state = tf.nn.dynamic_rnn(cell = LSTM_cell_decode, inputs = intermediate, dtype=tf.float32, scope = scope_decode)
	hidden = state[1]
	...
	p_s_true = tf.nn.softmax(tf.matmul(hidden, W_s)+b_s)
	p_s_false = 1-p_s_true
	p_s = tf.pack([p_s_false, p_s_true], axis = 2)

	p_e_true = tf.nn.softmax(tf.matmul(hidden, W_e)+b_e)
